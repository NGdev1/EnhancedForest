...
struct XInput {
    XInput()
        ...
        m_deadzone[GamepadAxis::LeftX] = m_deadzone[GamepadAxis::LeftY] = XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
        m_deadzone[GamepadAxis::RightX] = m_deadzone[GamepadAxis::RightY] = XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE;
        m_deadzone[GamepadAxis::LeftZ] = m_deadzone[GamepadAxis::RightZ] = XINPUT_GAMEPAD_TRIGGER_THRESHOLD;

        bx::memSet(m_flip, 1, sizeof(m_flip));
        m_flip[GamepadAxis::LeftY] = m_flip[GamepadAxis::RightY] = -1;
    }
    ...
    void update(EventQueue &_eventQueue) {
        ...
        for (uint16_t ii = 0; ii < BX_COUNTOF(m_state); ++ii) {
            ...
            if (connected && m_state[ii].dwPacketNumber != state.dwPacketNumber) {
...
struct Context {
    ...
    int32_t run(int _argc, const char *const *_argv) {
        ...
        m_windowAlloc.alloc();
        m_hwnd[0] = CreateWindowExA(WS_EX_ACCEPTFILES, "bgfx", "BGFX", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 0, 0, ENTRY_DEFAULT_WIDTH,
            ENTRY_DEFAULT_HEIGHT, NULL, NULL, instance, 0);
        ...
        if (m_init) {
            switch (_id) {
                case WM_USER_WINDOW_CREATE: {
                    Msg *msg = (Msg *)_lparam;
                    HWND hwnd = CreateWindowW(L"bgfx", UTF8ToUTF16(msg->m_title.c_str()).data(), WS_OVERLAPPEDWINDOW | WS_VISIBLE, msg->m_x,
                        msg->m_y, msg->m_width, msg->m_height, NULL, NULL, (HINSTANCE)GetModuleHandle(NULL), 0);
                ...
                case WM_SIZING: {
                    ...
                    if (isValid(handle) && ENTRY_WINDOW_FLAG_ASPECT_RATIO & m_flags[handle.idx]) {
