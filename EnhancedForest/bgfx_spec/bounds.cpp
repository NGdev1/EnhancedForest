...
bool overlap(const Aabb &_aabbA, const Aabb &_aabbB) {
    return true && _aabbA.max.x > _aabbB.min.x && _aabbB.max.x > _aabbA.min.x && _aabbA.max.y > _aabbB.min.y &&
           _aabbB.max.y > _aabbA.min.y && _aabbA.max.z > _aabbB.min.z && _aabbB.max.z > _aabbA.min.z;
}
...
bool overlap(const Capsule &_capsuleA, const Capsule &_capsuleB) {
    ...
    if (0.0f <= ta && 1.0f >= ta && 0.0f <= tb && 1.0f >= tb) {
        ...
        return overlap(Sphere{mad(ad, ta, _capsuleA.pos), _capsuleA.radius}, Sphere{mad(bd, tb, _capsuleB.pos), _capsuleB.radius});
    }
...
bool overlap(const Disk &_diskA, const Disk &_diskB) {
    ...
    return bx::sqrt(square(_diskA.radius) - square(lenA)) + bx::sqrt(square(_diskB.radius) - square(lenB)) >= distance(pa, pb);
}
...
bool overlap(const Plane &_plane, const Cone &_cone) {
    ...
    const Vec3 pos = add(add(_cone.end, mul(dir, len * bb / cc)), mul(v2, len * aa / cc));
...
bool overlap(const Triangle &_triangleA, const Triangle &_triangleB) {
    ...
    return overlap(_triangleA, _triangleB, cross(baA, cbA)) && overlap(_triangleA, _triangleB, cross(baB, cbB)) &&
           overlap(_triangleA, _triangleB, cross(baB, baA)) && overlap(_triangleA, _triangleB, cross(baB, cbA)) &&
           overlap(_triangleA, _triangleB, cross(baB, acA)) && overlap(_triangleA, _triangleB, cross(cbB, baA)) &&
           overlap(_triangleA, _triangleB, cross(cbB, cbA)) && overlap(_triangleA, _triangleB, cross(cbB, acA)) &&
           overlap(_triangleA, _triangleB, cross(acB, baA)) && overlap(_triangleA, _triangleB, cross(acB, cbA)) &&
